...
class ProductProduct(models.Model):
    ...
    def _compute_available_quantities_dict(self):
        ...
        # Use the standard method to retrieve the location domain
        domain_quant_loc, _, _ = self._get_domain_locations()
        orderpoint_obj = self.env["stock.location.orderpoint"]
        if orderpoint_obj._check_access_rights("read", raise_exception=False):
            orderpoints = orderpoint_obj.search(domain_quant_loc)
        else:
            for product in self:
